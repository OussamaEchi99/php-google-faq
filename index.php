...
<body>
    <?php 
        $paragraph = [
            'CGUE' => [
                ...
                "Per presentare una richiesta di rimozione, compila <a href=\"https://support.google.com/legal/contact/lr_eudpa?product=websearch&hl=it\">questo modulo web</a>. Riceverai una risposta automatica che conferma la ricezione della tua richiesta. Dopodiché valuteremo il tuo caso (tieni presente che potrebbe occorrere un po' di tempo perché abbiamo già ricevuto tante richieste simili). Nel valutare la richiesta, considereremo se i risultati includono informazioni obsolete relative alla tua vita privata. Considereremo inoltre se vi è un interesse pubblico a che le informazioni rimangano nei nostri risultati delle ricerche, ad esempio se riguardano frodi finanziarie, negligenza professionale, condanne penali o la tua condotta pubblica in relazione a un pubblico ufficio (eletto o non eletto). Queste valutazioni sono complesse e, in quanto organizzazione privata, potremmo non essere nella posizione giusta per prendere decisioni in merito al tuo caso. Se non sei d'accordo con la nostra valutazione, puoi rivolgerti all'Autorità garante per la protezione dei dati personali nel tuo paese.",
            ...
            'Protect_Privacy' => [
                ...
                'text1' => ["Sappiamo che la sicurezza e la privacy sono importanti per te e lo sono anche per noi. Per noi è imperativo offrire un'elevata sicurezza e assicurarti che le tue informazioni sono al sicuro e accessibili quando ne hai bisogno.",
            ...
            'Account' => [
                ...
                'text1' => "Il tuo account è associato a un paese (o territorio) nei Termini di servizio per poter stabilire due cose:",
                ...
                'text2' => "Tieni presente che i servizi Google sono fondamentalmente gli stessi a prescindere dalla società consociata che li offre o dal paese a cui è associato il tuo account.",
                'title2' => "Stabilire il paese associato al tuo account",
                'text3' => ["Quando crei un nuovo Account Google, lo associamo a un paese in base a dove è stato creato. Per quanto riguarda gli account creati almeno un anno fa, usiamo il paese da cui accedi solitamente ai servizi Google, in genere i servizi in cui hai trascorso più tempo nell'ultimo anno.",
    ...
    <header>
        <div>
            ...
            <div class="links_sections">
                ...
                <a href="">Termizi di servizio</a>
                <a href="">Tecnologie</a>
                <a href="">Damande frequenti</a>
            </div>
            <div></div>
        </div>
    </header>
    <main>
        <div class="container">
            <div>
                <?php foreach($paragraph['CGUE'] as $paragraph_section) { ?>
                    <h2><?php echo $paragraph_section['title'] ?></h2>
                    <?php foreach($paragraph_section as $text) { ?>
                        <p><?php echo '<p>' . $text . '</p>' ?></p>
                    <?php } ?>
                <?php } ?>
            </div>
            <div>
                <?php foreach($paragraph['Protect_Privacy'] as $paragraph_section) { ?>
                    <?php echo '<h2>' . $paragraph_section['title'] . '</h2>' ?>
                    <?php foreach($paragraph_section as $text) { ?>
                        <p><?php echo '<p>' . $text . '</p>' ?></p>
                    <?php } ?>
                <?php } ?>
            ...
            <div>
                <?php foreach($paragraph['Account'] as $paragraph_section) { ?>
                    <?php echo '<h2>' . $paragraph_section['title'] . '</h2>' ?>
                    <?php foreach($paragraph_section as $text) { ?>
            ...
            <div>
                <?php foreach($paragraph['Information'] as $paragraph_section) { ?>
                    <?php echo '<h2>' . $paragraph_section['title'] . '</h2>' ?>
                    <?php foreach($paragraph_section as $text) { ?>
                        <p><?php echo '<p>' . $text . '</p>' ?></p>
                    <?php } ?>
            ...
            <div>
                <?php foreach($paragraph['Google'] as $paragraph_section) { ?>
                    <?php echo '<h2>' . $paragraph_section['title'] . '</h2>' ?>
                    <?php foreach($paragraph_section as $text) { ?>
                        <p><?php echo '<p>' . $text . '</p>' ?></p>
                    <?php } ?>
